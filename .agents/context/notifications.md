...
> **TL;DR:** Bird is used for email only. WhatsApp is Evolution-only through
...
|---|---|---|
| Pending approver | Email via `notify_users_for_pending_status()` | Also sends `pending_approval` through Evolution |
| Generic final decision/status | Optional domain email | `request_status_update` through Evolution |
| Leave submitted | Email first; WhatsApp only when recipient has no email | Leave template through Evolution |
| Leave approved/rejected | Email first; WhatsApp only when employee has no email | Leave final-decision template through Evolution |
...
- Leave notification helpers intentionally use email first and send WhatsApp only when the recipient has no email address.
...
- Uses Ant Design `notification` for action toasts.
