...
- Protected binaries (payslip PDF, employee document PDF/image, announcement attachment) are approved as **web-app-only for the first mobile release**. `MobileApp` shows structured payslip details, employee document metadata, and announcement/attachment metadata only and must not fetch, render, persist, share, export, use a WebView, deep-link, or use signed URLs for protected binaries. `expo-file-system`, `expo-sharing`, and download helpers remain blocked by invariant. Future mobile viewing requires a separate security-reviewed design; Gate 4 overall is not passed.
