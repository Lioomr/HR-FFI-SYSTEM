# Late Attendance Notice Level 2 PDF Handoff

## Status

This is the approved design-stage asset for the second late-attendance occurrence. It is intentionally stored outside `Backend/static/pdf_templates` and does not change the backend. Do not move or implement it until the other notice levels are visually approved.

## Asset pair

- Blank template: `late_attendance_level_2_blank.pdf`
- Coordinate map: `late_attendance_level_2_blank_field_map.json`
- Approved visual reference: `approved-level-2-style-reference.png`
...
Use only the second-occurrence policy. The rendered notice must visibly include both:

- English: `Formal caution - 5% daily-rate deduction.`
- Arabic: `تنبيه رسمي - خصم بنسبة ٥٪ من الأجر اليومي.`

Do not add disciplinary, termination, legal, payroll-pending, or other consequences beyond the agreed 5% daily-rate deduction.

## Layout requirements preserved by this asset

- Bilingual English/Arabic title and Level 2 formal-caution badge.
- Deep amber/gold formal-caution accent with the approved FFI orange divider and contact footer.
...
```json
{
  "template": "late_attendance_level_2_blank.pdf",
  "version": 1,
...
The complete machine-readable map, including typography, colors, signature policy, and integration notes, is `late_attendance_level_2_blank_field_map.json`.
...
5. The approved visual has no separate blank zones for occurrence number, reason, penalty percentage/amount, or payroll status. Add those areas only after a specific Level 2 layout amendment is approved. Do not invent coordinates or cover the preprinted bilingual letter copy.
