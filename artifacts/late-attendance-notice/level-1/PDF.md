# Late Attendance Notice Level 1 PDF Handoff

## Status

This is the approved design-stage asset for the first late-attendance occurrence. It is intentionally stored outside `Backend/static/pdf_templates` and does not change the backend. Do not move or implement it until the other notice levels are visually approved.

## Asset pair

- Blank template: `late_attendance_level_1_blank.pdf`
- Coordinate map: `late_attendance_level_1_blank_field_map.json`
- Approved visual reference: `approved-level-1-style-reference.png`
...
Use only the first-occurrence policy. The rendered notice must visibly include both:

- English: `Warning only - no payroll deduction.`
- Arabic: `تحذير فقط - لا يوجد خصم من الراتب.`

Do not call the first occurrence pending, awaiting payroll, deducted, disciplinary, final, or legally consequential. The level is informational only.

## Layout requirements preserved by this asset

- Bilingual English/Arabic title and Level 1 informational-warning badge.
- Blue informational accent with the existing FFI orange divider and approved contact footer.
...
```json
{
  "template": "late_attendance_level_1_blank.pdf",
  "version": 1,
...
The complete machine-readable map, including typography, colors, signature policy, and integration notes, is `late_attendance_level_1_blank_field_map.json`.
...
5. The approved visual has no separate blank zones for occurrence number, reason, penalty percentage/amount, or payroll status. Add those areas only after a specific Level 1 layout amendment is approved. Do not invent coordinates or cover the preprinted bilingual letter copy.
