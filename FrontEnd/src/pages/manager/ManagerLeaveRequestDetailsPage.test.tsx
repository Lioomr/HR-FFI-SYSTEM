...
vi.mock("../../components/requests/RequestObligationsPanel", () => ({ default: () => null }));
    ...
    return render(
        <MemoryRouter initialEntries={["/manager/leave/requests/9"]}>
            <Routes>
                ...
                />
                <Route path="/manager/team-requests" element={<div>Team requests</div>} />
            </Routes>
        </MemoryRouter>,
    );
}
...
describe("ManagerLeaveRequestDetailsPage", () => {
    ...
    it("offers approve and reject while the manager is the deciding approver", async () => {
        ...
        expect(await screen.findByRole("button", { name: "Approve: Sara Idris" })).toBeInTheDocument();
        expect(screen.getByRole("button", { name: "Reject: Sara Idris" })).toBeInTheDocument();
    });

    it("hides the decision controls once the request has moved on", async () => {
        mockedFetch.mockResolvedValue({ status: "success", data: { ...BASE, status: "pending_hr" } });

        renderPage();

        expect(
            await screen.findByText("This request is no longer waiting for your decision."),
        ).toBeInTheDocument();
        expect(screen.queryByRole("button", { name: "Approve: Sara Idris" })).not.toBeInTheDocument();
        expect(screen.queryByRole("button", { name: "Reject: Sara Idris" })).not.toBeInTheDocument();
    });
    ...
    it("confirms an approval through a dialog", async () => {
        ...
        fireEvent.click(await screen.findByRole("button", { name: "Approve: Sara Idris" }));
    ...
    it("requires a written reason before rejecting", async () => {
        ...
        fireEvent.click(await screen.findByRole("button", { name: "Reject: Sara Idris" }));
        ...
        await waitFor(() => expect(mockedReject).toHaveBeenCalledWith(9, "Peak season"));
    });
    ...
    it("shows a no-permission state for a request outside the manager's team", async () => {
        ...
        expect(await screen.findByText("You cannot review this request")).toBeInTheDocument();
        expect(
            screen.getByText(
