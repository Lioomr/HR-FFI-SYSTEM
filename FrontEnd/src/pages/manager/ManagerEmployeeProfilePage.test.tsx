...
function renderPage() {
    return render(
        <MemoryRouter initialEntries={["/manager/team/11"]}>
            <Routes>
                <Route path="/manager/team/:id" element={<ManagerEmployeeProfilePage />} />
...
describe("ManagerEmployeeProfilePage", () => {
    ...
    it("shows identity, contact, employment, leave balances and open requests", async () => {
        ...
        expect(screen.getByRole("tab", { name: /Employment Info/ })).toBeInTheDocument();
        expect(screen.getByRole("tab", { name: /Leave Balances/ })).toBeInTheDocument();
    ...
    it("never renders a salary or allowance amount, even after opening every tab", async () => {
        ...
        SALARY_VALUES.forEach((value) => {
            expect(body, `salary amount ${value} leaked into the manager profile`).not.toContain(
                value,
            );
        });
