      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            label: tabLabel(
              "asset-returns",
              t("manager.requests.assetReturnsTab"),
            ),
            forceRender: true,
            children: (
              <AssetReturnRequestsTab
                refreshToken={refreshToken}
                onCount={reportCount}
              />
            ),
...
function LeaveRequestsTab({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isApiError(res)) {
        notification.error({
          message: t("manager.requests.loadFailed"),
  ...
  const pendingCount = useMemo(
    () =>
      data.filter((row) =>
        LEAVE_ACTIONABLE.has(String(row.status || "").toLowerCase()),
      ).length,
    [data],
  );
  ...
  const approve = async (row: ManagerLeaveRequest) => {
    ...
    // Optimistic: drop the row from the actionable queue immediately instead
    // of waiting for the round trip; restore it if the request fails.
    ...
    try {
      ...
      if (isApiError(res)) {
        ...
        notification.error({
          message: t("manager.requests.actionFailed"),
          description: res.message,
        });
        return;
      }
      notification.success({ message: t("manager.requests.approveSuccess") });
      // Reconcile the optimistic row removal with the authoritative
      // queue, counts, and any decisions made by another approver.
      await load();
    } catch {
      setData((current) =>
        current.some((item) => item.id === row.id)
  ...
  const reject = async (reason: string) => {
    ...
    setData((current) => current.filter((item) => item.id !== row.id));
    try {
      const res = await rejectLeaveRequestManager(row.id, reason);
      if (isApiError(res)) {
        setData((current) =>
          current.some((item) => item.id === row.id)
            ? current
  ...
  const columns: ColumnsType<ManagerLeaveRequest> = [
    ...
    },
    {
      ...
      width: 290,
      render: (_, record) => {
        const actionable = LEAVE_ACTIONABLE.has(
        ...
        <Table
          ...
          expandable={{
            expandedRowRender: (record) => (
              <LeaveApprovalMap request={record as any} t={t} />
            ),
          }}
          scroll={{ x: 1280 }}
          pagination={{
...
function AssetReturnRequestsTab({
  ...
  const reject = async (reason: string) => {
    ...
    setData((current) => current.filter((item) => item.id !== row.id));
    try {
      const res = await rejectManagerAssetReturnRequest(row.id, reason);
      if (isApiError(res)) {
        setData((current) =>
          current.some((item) => item.id === row.id)
            ? current
...
function TeamTab({ refreshToken }: { refreshToken: number }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isApiError(res)) {
        notification.error({
          message: t("manager.team.failedLoad"),
          description: res.message,
        });
        return;
      }
