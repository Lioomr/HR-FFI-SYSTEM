...
vi.mock("../../services/api/attendanceCorrectionsApi", async (importOriginal) => ({
    ...(await importOriginal<typeof import("../../services/api/attendanceCorrectionsApi")>()),
...
function renderPage(path = "/manager/team-requests") {
    return render(
        <MemoryRouter initialEntries={[path]}>
            <Routes>
                <Route path="/manager/team-requests" element={<ManagerTeamRequestsPage />} />
            </Routes>
        </MemoryRouter>,
    );
}
...
describe("ManagerTeamRequestsPage", () => {
    ...
    beforeEach(() => {
        ...
        mockedLeave.mockResolvedValue({ status: "success", data: [PENDING_LEAVE, DECIDED_LEAVE] });
    ...
    it("shows the outstanding count per tab and totals them in the header", async () => {
        ...
        expect(screen.getByRole("tab", { name: /Leave Requests/ })).toHaveTextContent("1");
        expect(screen.getByRole("tab", { name: /Attendance Corrections/ })).toHaveTextContent("2");
    });
    ...
    it("rejects through a modal with a required reason, never window.prompt", async () => {
        ...
        expect(within(dialog).getByText("Reject leave request")).toBeInTheDocument();
    ...
    it("filters the leave queue by employee", async () => {
        ...
        fireEvent.change(screen.getByRole("textbox", { name: /Search by employee/ }), {
            target: { value: "omar" },
        });
