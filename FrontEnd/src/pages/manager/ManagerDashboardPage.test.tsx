...
function makeItem(
  overrides: Partial<ManagerPendingItem> & { queue: ManagerQueueKey },
): ManagerPendingItem {
  return {
...
describe("ManagerDashboardPage", () => {
  ...
  it("reports the managed employee count and the pending total", async () => {
    ...
    expect(
      await screen.findByText("3 employees report to you"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("button", { name: /Direct reports: 3/ }),
    ).toBeInTheDocument();
  });
  ...
  it("offers the review call to action only while something is pending", async () => {
    mockedSummary.mockResolvedValue(
      makeSummary([makeItem({ queue: "leave", id: 1 })]),
    );

    render(<ManagerDashboardPage />);

    fireEvent.click(
      await screen.findByRole("button", { name: "Review pending requests" }),
    );
  ...
  it("renders a retryable error when no queue can be read", async () => {
    ...
    expect(
      await screen.findByText("Could not load your team overview"),
    ).toBeInTheDocument();
  ...
  it("tells a capability holder with no assigned employees that nobody reports to them", async () => {
    ...
    expect(
      await screen.findByText("No employees assigned to you yet"),
    ).toBeInTheDocument();
  ...
  it("renders Arabic labels when the language is Arabic", async () => {
    ...
    expect(
      await screen.findByText("قائمة العمل حسب الأولوية"),
    ).toBeInTheDocument();
