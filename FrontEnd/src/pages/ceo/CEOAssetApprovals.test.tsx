...
const getDamage = assetsApi.getCEOAssetDamageReports as unknown as ReturnType<typeof vi.fn>;
const approveDamage = assetsApi.approveCEOAssetDamageReport as unknown as ReturnType<typeof vi.fn>;
const rejectDamage = assetsApi.rejectCEOAssetDamageReport as unknown as ReturnType<typeof vi.fn>;
const getReturns = assetsApi.getCEOAssetReturnRequests as unknown as ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  [getDamage, approveDamage, rejectDamage, getReturns].forEach((fn) => fn.mockReset());
...
describe("CEO asset approval queues", () => {
  it("renders damage reports with a labelled status and decision pair", async () => {
    ...
    expect(screen.getByRole("button", { name: "Approve: Sara Ahmed" })).toBeInTheDocument();
  ...
  it("shows the empty state when no damage report is awaiting approval", async () => {
    ...
    expect(await screen.findByText("No damage reports awaiting approval")).toBeInTheDocument();
  });

  it("shows a retryable error state when the queue fails", async () => {
    getDamage.mockResolvedValue({ status: "error" as const, message: "asset service down" });
  ...
  it("confirms an approval and reloads the queue", async () => {
    getDamage.mockResolvedValue(ok([damageRow]));
    approveDamage.mockResolvedValue({ status: "success" as const, data: damageRow });
    ...
    await waitFor(() => expect(approveDamage).toHaveBeenCalledWith(12, "Approved by CEO"));
  ...
  it("sends the CEO's own note when one is written", async () => {
    ...
    await waitFor(() => expect(approveDamage).toHaveBeenCalledWith(12, "Replace under warranty"));
  });

  it("blocks a rejection until a reason is written", async () => {
    getDamage.mockResolvedValue(ok([damageRow]));
    rejectDamage.mockResolvedValue({ status: "success" as const, data: damageRow });
    ...
    await waitFor(() => expect(rejectDamage).toHaveBeenCalledWith(12, "Employee negligence"));
  });
  ...
  it("keeps the approval map available on return requests", async () => {
    ...
    expect(await screen.findByTestId("asset-return-approval-map")).toBeInTheDocument();
  });
