...
export default function CEOTeamPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!isApiError(res) && res.data) {
          ...
          notification.error({ message: t("ceo.team.failedLoad") });
        }
      } catch {
        notification.error({ message: t("ceo.team.failedLoad") });
      } finally {
  ...
  const columns = [
    ...
    { title: t("ceo.team.roleEmail"), dataIndex: "email", key: "email", render: (v: string) => v || "—" },
  ...
  return (
    <div>
      <PageHeader title={t("ceo.team.title")} subtitle={t("ceo.team.subtitle")} />
