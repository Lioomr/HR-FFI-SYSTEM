...
vi.mock("../../../services/api/employeesApi", () => ({
    listDelegationCandidates: vi.fn().mockResolvedValue({ status: "success", data: [] }),
}));
...
describe("employee leave backdated dates", () => {
    it("enables exactly 7 days ago", () => {
        expect(isEmployeeLeaveDateDisabled(today.subtract(7, "day"), today)).toBe(false);
    });

    it("disables 8 days ago", () => {
        expect(isEmployeeLeaveDateDisabled(today.subtract(8, "day"), today)).toBe(true);
    });
    ...
    it("enables future dates", () => {
        expect(isEmployeeLeaveDateDisabled(today.add(30, "day"), today)).toBe(false);
    });
    ...
    it("renders the translated helper text", async () => {
        ...
        expect(
            await screen.findByText("Leave can be submitted up to 7 calendar days after it starts."),
        ).toBeInTheDocument();
    });
    ...
    it("renders the backend start-date validation error clearly", () => {
        const error = {
            apiData: {
                errors: ["start_date: Leave can be submitted up to 7 calendar days after it starts."],
            },
        };
