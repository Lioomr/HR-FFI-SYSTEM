...
export default function RequestLeavePage() {
  ...
  const getBalanceByCode = (code: string): LeaveBalance | undefined => {
    ...
    const leaveType = leaveTypes.find(
      (t) => (t.code || "").toUpperCase() === normalized,
    );
  ...
  const getAvailableDaysForType = (typeObj: LeaveType): number => {
    ...
    if (["ANNUAL", "ANNUAL_LEAVE"].includes(code)) {
      const unpaidRemaining = Number(
        getBalanceByCode("UNPAID")?.remaining_days || 0,
      );
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        const employeesRes = await listDelegationCandidates({ scope: "all" });
