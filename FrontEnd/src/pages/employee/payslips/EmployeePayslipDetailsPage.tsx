...
} from "../../../services/api/employeePayslipsApi";
...
export default function EmployeePayslipDetailsPage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      triggerBlobDownload(blob, `Payslip_${payslip.month}_${payslip.year}.pdf`);
    } catch {
  ...
  if (!payslip)
    return (
      <ErrorState
        ...
        description={t("payslips.details.notFound")}
      />
    );
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      ...
      <Card style={{ borderRadius: 16 }}>
        <Descriptions
          title={t("payslips.details.summary")}
          bordered
          column={1}
          labelStyle={{ width: 200 }}
        >
          <Descriptions.Item label={t("payslips.list.colPeriod")}>
          ...
          {payslip.payment_mode && (
            <Descriptions.Item label={t("payslips.list.colPaymentMode")}>
              {(() => {
                const key = payslip.payment_mode
                  .toLowerCase()
                  .replace(/\s+/g, "");
          ...
          <Descriptions.Item label={t("payslips.details.generatedDate")}>
            {formatDateOnly(payslip.generated_at)}
          </Descriptions.Item>
        </Descriptions>
        ...
        <Descriptions
          title={t("payslips.details.earnings")}
          bordered
          column={1}
          labelStyle={{ width: 200 }}
        >
          <Descriptions.Item label={t("payroll.details.colBasicSalary")}>
            <AmountWithSAR amount={payslip.basic_salary} />
          </Descriptions.Item>

          {/* Allowances Breakdown */}
          <Descriptions.Item label={t("payslips.details.transportation")}>
            <AmountWithSAR amount={payslip.transportation_allowance} />
          </Descriptions.Item>
          <Descriptions.Item label={t("payslips.details.accommodation")}>
            <AmountWithSAR amount={payslip.accommodation_allowance} />
          </Descriptions.Item>
          <Descriptions.Item label={t("payslips.details.telephone")}>
            <AmountWithSAR amount={payslip.telephone_allowance} />
          </Descriptions.Item>
          <Descriptions.Item label={t("payslips.details.petrol")}>
            <AmountWithSAR amount={payslip.petrol_allowance} />
          </Descriptions.Item>
          <Descriptions.Item label={t("payslips.details.other")}>
            <AmountWithSAR amount={payslip.other_allowance} />
          </Descriptions.Item>

          <Descriptions.Item
            label={t("payslips.details.totalAllowances")}
            contentStyle={{ fontWeight: "bold" }}
          >
            <AmountWithSAR
              amount={payslip.total_allowances}
              fontWeight="bold"
            />
          </Descriptions.Item>

          <Descriptions.Item
            label={t("payslips.details.grossSalary")}
            contentStyle={{ fontWeight: "bold" }}
          >
            <AmountWithSAR
              amount={
                payslip.total_salary ??
                payslip.basic_salary + payslip.total_allowances
              }
              fontWeight="bold"
            />
          </Descriptions.Item>
        ...
        <Descriptions
          title={t("payslips.details.deductions")}
          bordered
          column={1}
          labelStyle={{ width: 200 }}
        >
          <Descriptions.Item
            label={t("payslips.details.totalDeductions")}
