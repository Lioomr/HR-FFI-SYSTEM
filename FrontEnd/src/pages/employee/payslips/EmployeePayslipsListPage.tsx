...
export default function EmployeePayslipsListPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (isApiError(res)) {
        notification.error({
          message: t("payslips.list.failedLoad"),
  ...
  const handleDownload = async (item: EmployeePayslip) => {
    ...
    try {
      ...
      triggerBlobDownload(blob, `Payslip_${item.month}_${item.year}.pdf`);
    } catch {
      notification.error({
        message: t("payslips.list.downloadFailed"),
        description: t("payslips.list.couldNotDownloadPdf"),
      });
    } finally {
  ...
  const columns: ColumnsType<EmployeePayslip> = [
    {
      ...
      width: 160,
      render: (_, r) => {
        const monthName = new Date(0, r.month - 1).toLocaleString(
    ...
    },
    {
      ...
      render: (status) => {
        ...
        return (
          <Tag color={s === "paid" ? "green" : "blue"}>{t(`status.${s}`)}</Tag>
        );
      },
    },
    {
