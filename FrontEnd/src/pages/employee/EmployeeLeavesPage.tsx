  ...
  return (
    <div>
      ...
      >
        ...
        <ul>
          <li>
            <strong>{t("leaves.openingBalance")}</strong>{" "}
            {t("leaves.openingBalanceDesc")}
          </li>
          <li>
            <strong>{t("leaves.used")}</strong> {t("leaves.usedDesc")}
          </li>
          <li>
            <strong>{t("leaves.remaining")}:</strong>{" "}
            {t("leaves.remainingDesc")}
          </li>
        </ul>
      </div>
      ...
      >
          ...
          onValuesChange={(changedValues) => {
            if (changedValues.leave_type) {
              const selected = balances.find(
                (b) => b.leave_type_id === changedValues.leave_type,
              );
              const code =
                (selected as any)?.leave_code || selected?.leave_type || "";
        ...
        >
          ...
          >
            <Select placeholder={t("leaves.selectLeaveTypePlaceholder")}>
              {balances.map((balance) => (
                ...
                >
                  {t(
                    `leave.balance.${balance.leave_type.toLowerCase().replace(/\s+/g, ".")}`,
                    balance.leave_type,
                  )}{" "}
                  ({t("leaves.remaining")}: {balance.remaining_days}{" "}
                  {t("leaves.days")})
                </Select.Option>
              ))}
            </Select>
          ...
          <Form.Item name="reason" label={t("leaves.reasonOptional")}>
            <TextArea
              rows={4}
          ...
          <Form.Item
            ...
            label={
              isSickSelected
                ? t("leaves.medicalReportReq")
                : t("leaves.documentOptional")
            }
            ...
            rules={[
              {
                validator: (_, value: UploadFile[]) => {
                  ...
                  return value && value.length > 0
                    ? Promise.resolve()
                    : Promise.reject(
                        new Error(t("leaves.uploadMedicalReport")),
                      );
                },
              },
            ]}
