...
export default function EmployeeLoanRequestDetailsPage() {
  ...
  const getLoanTypeLabel = (loanType?: string) => {
    if (loanType === "installment")
      return t("loans.request.loanTypeInstallment");
        ...
        {!item ? null : (
          <>
            ...
            >
              ...
              <Descriptions.Item
                label={t("loans.request.monthlyDeductionLabel")}
              >
                <AmountWithSAR amount={monthlyDeduction} />
              </Descriptions.Item>
              <Descriptions.Item
                label={t("loans.details.targetDeductionPeriod")}
              >
              ...
              <Descriptions.Item label={t("loans.list.colReason")} span={2}>
                {item.reason || "-"}
              </Descriptions.Item>
              ...
              {item.approved_amount && (
                <Descriptions.Item
                  label={t("loans.details.approvedAmount") || "Approved Amount"}
                >
                  <AmountWithSAR
              ...
              <div style={{ position: "relative", paddingLeft: 4 }}>
                ...
                <div
                  style={{
                    ...
                    width: 2,
                    background:
                      "linear-gradient(to bottom, #f97316 0%, #e2e8f0 25%)",
                    opacity: 0.35,
                    borderRadius: 2,
                  }}
                />
                ...
                {(item.decision_history || []).map((entry, index) => {
                  ...
                  if (stageLower.includes("submit")) {
                    ...
                    stageLower.includes("reject") ||
                    noteLower.includes("reject")
                  ) {
                    dotColor = "#ef4444";
                    icon = <CloseCircleFilled style={{ fontSize: 12 }} />;
                  } else if (
                    stageLower.includes("approve") ||
                    noteLower.includes("approv")
                  ) {
                    dotColor = "#10b981";
                    icon = <CheckCircleFilled style={{ fontSize: 12 }} />;
                    ...
                    >
                      <div
                        style={{
                          ...
                          flexShrink: 0,
                          marginTop: 2,
                          boxShadow: isFirst ? `0 0 8px ${dotColor}30` : "none",
                        }}
                      >
                        <div style={{ color: dotColor, lineHeight: 0 }}>
                          {icon}
                        </div>
                      </div>
