  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (!isApiError(empResponse)) {
        ...
        empResponse.message.includes("404")
      ) {
                ...
                items={[
                  {
                    ...
                    label: (
                      <span>
                        <UserOutlined /> {t("profile.personalInfo")}
                      </span>
                    ),
                    children: (
                      <Descriptions
                      ...
                      >
                        ...
                        <Descriptions.Item label={t("profile.dateOfBirth")}>
                        ...
                        <Descriptions.Item label={t("profile.directManager")}>
                          {formatValue(directManagerName)}
                        </Descriptions.Item>
                      </Descriptions>
                    ),
                  },
                  {
                    ...
                    label: (
                      <span>
                        <ContainerOutlined /> {t("profile.employmentInfo")}
                      </span>
                    ),
                    children: (
                      <Descriptions
                      ...
                      >
                        ...
                        <Descriptions.Item label={t("profile.taskGroup")}>
                          {formatValue(employee.task_group)}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.sponsor")}>
                          {formatValue(employee.sponsor)}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.jobOffer")}>
                          {formatValue((employee as any).job_offer)}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.joiningDate")}>
                          {formatDate(
                            (employee as any).join_date || employee.hire_date,
                          )}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.contractDate")}>
                          {formatDate((employee as any).contract_date)}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.contractExpiry")}>
                          {formatDate((employee as any).contract_expiry)}
                        </Descriptions.Item>
                        <Descriptions.Item label={t("profile.allowedOvertime")}>
                  ...
                  },
                  {
                    ...
                    label: (
                      <span>
                        <DollarOutlined /> {t("profile.salaryDetails")}
                      </span>
                    ),
                    children: (
                      <div>
                        <Descriptions
                          ...
                          style={{ marginTop: 16 }}
                        >
                          <Descriptions.Item label={t("profile.basicSalary")}>
                        ...
                        >
                          <Descriptions.Item
                            label={t("profile.transportation")}
                          >
                            {formatCurrency(
                              (employee as any).transportation_allowance,
                            )}
                          </Descriptions.Item>
                          <Descriptions.Item label={t("profile.accommodation")}>
                            {formatCurrency(
                              (employee as any).accommodation_allowance,
                            )}
                          </Descriptions.Item>
                          <Descriptions.Item label={t("profile.telephone")}>
                            {formatCurrency(
                              (employee as any).telephone_allowance,
                            )}
                          </Descriptions.Item>
                          <Descriptions.Item label={t("profile.petrol")}>
                            {formatCurrency((employee as any).petrol_allowance)}
                          </Descriptions.Item>
                          <Descriptions.Item label={t("profile.other")}>
                    ...
                    ? [
                        {
                          ...
                          label: (
                            <span>
                              <ScheduleOutlined />{" "}
                              {t("profile.leaveBalances", "Leave Balances")}
                            </span>
                          ),
                          children: (
                            <div style={{ marginTop: 16 }}>
                              {employee.leave_balance_year && (
                                ...
                                >
                                  {t("profile.leaveBalanceYear", "Year")}:{" "}
                                  <strong>{employee.leave_balance_year}</strong>
                                </Typography.Text>
                              )}
                              <LeaveBalanceTable
                                balances={
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <Title level={3} style={{ margin: 0 }}>
                {user.full_name || t("profile.systemUser")}
              </Title>
              <div
                style={{
                  marginTop: 8,
              ...
              <div style={{ marginTop: 8 }}>
                <Space>
                  <MailOutlined style={{ color: "#bfbfbf" }} />
                  <a href={`mailto:${user.email}`} style={{ fontSize: 13 }}>
                    {user.email}
                  </a>
                </Space>
              </div>
            ...
            >
              <Descriptions column={1} bordered size="small">
                ...
                <Descriptions.Item
                  label={
                    <span style={{ color: "#8c8c8c" }}>
                      <CalendarOutlined /> {t("profile.joined")}
                    </span>
                  }
                >
                ...
                <Descriptions.Item
                  label={
                    <span style={{ color: "#8c8c8c" }}>
                      <SafetyCertificateOutlined /> {t("profile.status")}
                    </span>
                  }
                >
                  <Tag color={user.is_active ? "success" : "error"}>
