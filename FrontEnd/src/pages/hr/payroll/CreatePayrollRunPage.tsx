    ...
    try {
      ...
      if (isApiError(response)) {
        ...
        if (
          ...
          try {
            const listRes = await getPayrollRuns({
              year: values.year,
              page_size: 100,
            });
            if (!isApiError(listRes)) {
              ...
              const existing = listRes.data.items.find(
                (r) => r.month === values.month && r.year === values.year,
              );
              if (existing) {
                notification.info({
      ...
      const duplicateDetected =
        ...
        (err.response?.status === 422 &&
          (backendMessage.includes("already exists") ||
            backendErrors.includes("already exists") ||
            backendErrors.includes("unique")));
        ...
        try {
          ...
          if (!isApiError(listRes) && listRes.data?.items) {
            ...
            if (existing) {
              notification.info({
                message: t("payroll.runExists"),
                description: t("payroll.runExistsDesc2", {
                  month: values.month,
                  year: values.year,
                }),
                duration: 3,
