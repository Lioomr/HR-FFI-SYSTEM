...
export default function PayrollRunDetailsPage() {
  ...
  const columns: ColumnsType<PayrollRunItem> = [
    ...
    },
    {
      ...
      align: "right",
      render: (val) => (
        <AmountWithSAR amount={val} size={12} fontWeight="bold" />
      ),
    },
  ];
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (isApiError(runRes)) {
        throw new Error(
          runRes.message || t("payroll.runDetails.failedLoadDetails"),
        );
      }
      if (isApiError(itemsRes)) {
        throw new Error(
          itemsRes.message || t("payroll.runDetails.failedLoadItems"),
        );
      }
      if (isApiError(summaryRes)) {
        throw new Error(
          summaryRes.message || t("payroll.runDetails.failedLoadDetails"),
        );
      }
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (isApiError(res)) {
            notification.error({
              message: t("payroll.runDetails.finalizeFailed"),
          ...
          notification.error({
            ...
            description: e.message || t("payroll.runDetails.finalizeFailed"),
          });
        } finally {
          setFinalizing(false);
        }
  ...
  if (loading && !run)
    return <LoadingState title={t("payroll.runDetails.loadingDetails")} />;
  ...
  if (!run)
    return (
      <EmptyState
        ...
        description={t("payroll.runDetails.notFound")}
        actionText={t("payroll.runDetails.backToDashboard")}
        ...
        actions={
          ...
          >
            <Tag color={isFinalized ? "green" : "orange"}>
              {isFinalized ? (
                <>
                  <CheckCircleOutlined /> {run.status}
                </>
              ) : (
                run.status
              )}
                ...
                <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                  <Col span={summaryColumnSpan}>
                    <Card>
                      ...
                      >
                        ...
                        <Descriptions.Item
                          label={t("payroll.runDetails.totalEmployees")}
                        >
                          {run.total_employees}
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={t(
                            "payroll.runDetails.employeesWithDeductions",
                          )}
                        >
                  ...
                  <Col span={summaryColumnSpan}>
                    <Card>
                      ...
                      >
                        <Descriptions.Item
                          label={t("payroll.details.colBasicSalary")}
                        >
                          <AmountWithSAR
                            amount={summary?.total_basic_salary ?? 0}
                            size={14}
                          />
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={t("payroll.runDetails.colAllowances")}
                        >
                          <AmountWithSAR
                            amount={summary?.total_allowances ?? 0}
                            size={14}
                          />
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={t("payroll.runDetails.totalGrossSalary")}
                        >
                          <AmountWithSAR
                            amount={summary?.total_gross_salary ?? 0}
                            size={14}
                          />
                        </Descriptions.Item>
                      </Descriptions>
                  ...
                  <Col span={summaryColumnSpan}>
                    <Card>
                      ...
                      >
                        <Descriptions.Item
                          label={t("payroll.runDetails.colDeductions")}
                        >
                          <AmountWithSAR
                            amount={summary?.total_deductions ?? 0}
                            size={14}
                            color="red"
                          />
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={t("payroll.runDetails.totalNetSalary")}
                        >
                          <AmountWithSAR
                        ...
                        <Descriptions.Item
                          label={t("payroll.runDetails.averageNetSalary")}
                        >
                          <AmountWithSAR
                            amount={summary?.average_net_salary ?? 0}
                            size={14}
                          />
                        </Descriptions.Item>
                      </Descriptions>
