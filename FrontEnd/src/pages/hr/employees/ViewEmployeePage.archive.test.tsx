...
vi.mock("../../../services/api/usersApi", () => ({
  listUsers: vi.fn().mockResolvedValue({ status: "success", data: { results: [] } }),
}));
...
const getEmployee = employeesApi.getEmployee as unknown as ReturnType<typeof vi.fn>;
...
describe("ViewEmployeePage archived employee", () => {
  it("renders the work license expiry on the employee profile", async () => {
    ...
    const workLicenseCard = await screen.findByTestId("work-license-expiry-card");
  ...
  it("shows the archived banner with the reason and archive date", async () => {
    ...
    expect(await screen.findByText("This employee is archived")).toBeInTheDocument();
    expect(
    ...
    expect(screen.getByText(/Archived by:\s*Archive Administrator/)).toBeInTheDocument();
  });
  ...
  it("never renders the raw archiver id", async () => {
    ...
    const banner = screen.getByText("This employee is archived").closest(".ant-alert") as HTMLElement;
  ...
  it("does not show the archived banner for an active employee", async () => {
    ...
    expect(screen.queryByText("This employee is archived")).not.toBeInTheDocument();
  });

  it("restores the employee and reloads the profile", async () => {
    restoreEmployee.mockResolvedValue({
      status: "success",
      data: makeEmployee({ is_archived: false, archive_reason: null, archived_at: null }),
    });
  ...
  it("shows the backend error message when the restore fails", async () => {
    restoreEmployee.mockResolvedValue({ status: "error", message: "Employee is not archived." });
    ...
    expect(await within(dialog).findByText("Employee is not archived.")).toBeInTheDocument();
  });
  ...
  it("hides the restore action from roles that cannot restore", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Restore Employee" })).not.toBeInTheDocument();
  });
});
