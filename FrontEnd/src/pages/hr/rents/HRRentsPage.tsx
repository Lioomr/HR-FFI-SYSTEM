...
export default function HRRentsPage() {
  ...
  const [rentTypeFilter, setRentTypeFilter] = useState<number | undefined>(
    undefined,
  );
  ...
  const loadReferenceData = useCallback(async () => {
    try {
      const [rentTypeRes, assetRes] = await Promise.all([
        listRentTypes(),
      ...
      message.error(
        err.message ||
          t("hr.rents.errorLoadRefs", "Failed to load reference data"),
      );
    }
  }, [t]);
    ...
    async (nextPage = page, nextPageSize = pageSize) => {
      ...
      try {
        ...
        if (isApiError(response)) {
          message.error(
            response.message || t("hr.rents.errorLoad", "Failed to load rents"),
          );
          setLoading(false);
          return;
        }
        ...
        if (isForbidden(err)) {
          ...
          message.error(
            err.message || t("hr.rents.errorLoad", "Failed to load rents"),
          );
        }
      } finally {
        setLoading(false);
  ...
  const toPayload = (values: RentFormValues): CreateRentDto => ({
    ...
    one_time_due_date:
      values.recurrence === "ONE_TIME"
        ? values.one_time_due_date?.format("YYYY-MM-DD") || null
        : null,
    start_date:
      values.recurrence === "MONTHLY"
        ? values.start_date?.format("YYYY-MM-DD") || null
        : null,
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (isApiError(response)) {
        message.error(
          response.message ||
            t("hr.rents.errorCreate", "Failed to create rent"),
        );
        setSubmitting(false);
        return;
      }
      ...
      if (!err?.errorFields)
        message.error(
          err.message || t("hr.rents.errorCreate", "Failed to create rent"),
        );
    } finally {
      setSubmitting(false);
    }
  ...
  const openEdit = (item: RentItem) => {
    ...
    editForm.setFieldsValue({
      ...
      lease_start_date: item.lease_start_date
        ? dayjs(item.lease_start_date)
        : undefined,
      lease_end_date: item.lease_end_date
        ? dayjs(item.lease_end_date)
        : undefined,
      annual_rent_value:
        item.annual_rent_value === null
          ? undefined
          : Number(item.annual_rent_value),
      security_deposit:
      ...
      one_time_due_date:
        item.recurrence === "ONE_TIME" && item.one_time_due_date
          ? dayjs(item.one_time_due_date)
          : undefined,
      start_date:
        item.recurrence === "MONTHLY" && item.start_date
          ? dayjs(item.start_date)
          : undefined,
      due_day:
        item.recurrence === "MONTHLY" ? item.due_day || undefined : undefined,
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (isApiError(response)) {
        message.error(
          response.message ||
            t("hr.rents.errorUpdate", "Failed to update rent"),
        );
        setSubmitting(false);
        return;
      }
      ...
      if (!err?.errorFields)
        message.error(
          err.message || t("hr.rents.errorUpdate", "Failed to update rent"),
        );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async (id: number) => {
    ...
    if (isApiError(response)) {
      message.error(
        response.message || t("hr.rents.errorDelete", "Failed to delete rent"),
      );
      return;
    }
  ...
  const handleNotify = async (id: number) => {
    ...
    try {
      ...
      if (isApiError(response)) {
        message.error(
          response.message ||
            t("hr.rents.errorNotify", "Failed to send reminder"),
        );
        return;
      }
      ...
      message.error(
        err.message || t("hr.rents.errorNotify", "Failed to send reminder"),
      );
    } finally {
      setNotifyingId(null);
    }
  ...
  const handleAddPayment = async () => {
    ...
    try {
      ...
      const payload: CreateRentPaymentDto = {
        ...
        paid_date:
          values.status === "paid"
            ? values.paid_date?.format("YYYY-MM-DD") || null
            : null,
        notes: values.notes || "",
      };
      ...
      if (isApiError(response)) {
        message.error(
          response.message ||
            t("hr.rents.payments.errorCreate", "Failed to add payment record"),
        );
        return;
      }
      ...
      const updatedViewing = {
        ...viewing,
        payment_records: [
          ...(viewing.payment_records || []),
          response.data,
        ].sort((a, b) => a.payment_number - b.payment_number || a.id - b.id),
      };
      setViewing(updatedViewing);
      setItems((current) =>
        current.map((item) =>
          item.id === updatedViewing.id ? updatedViewing : item,
        ),
      );
      ...
      message.success(
        t("hr.rents.payments.successCreate", "Payment record added"),
      );
    } catch (err: any) {
      if (!err?.errorFields)
        message.error(
  ...
  const recurrenceOptions = useMemo(
    () => [
      {
        label: t("hr.rents.recurrence.oneTime", "One Time"),
        value: "ONE_TIME",
      },
  ...
  const paymentCategoryOptions = useMemo(
    () => [
      ...
      {
        label: t(
          "hr.rents.payments.category.securityDeposit",
          "Security Deposit",
        ),
  ...
  const paymentStatusOptions = useMemo(
    () => [
      {
        label: t("hr.rents.payments.status.pending", "Pending"),
        value: "pending",
      },
      { label: t("hr.rents.payments.status.paid", "Paid"), value: "paid" },
      {
        label: t("hr.rents.payments.status.cancelled", "Cancelled"),
        value: "cancelled",
      },
    ],
    [t],
  ...
  const getRentName = (record: RentItem) => {
    if (record.asset)
      return language === "ar"
        ? record.asset.name_ar || record.asset.name_en
        : record.asset.name_en;
    return language === "ar"
      ? record.property_name_ar || record.property_name_en || "-"
      : record.property_name_en || "-";
  };
  ...
  const getPaymentCategoryLabel = (category: RentPaymentCategory) =>
    paymentCategoryOptions.find((option) => option.value === category)?.label ||
    category;

  const getPaymentStatusTag = (status: RentPaymentStatus) => {
    const color =
      status === "paid" ? "green" : status === "cancelled" ? "red" : "gold";
    return (
      <Tag color={color}>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: RentItem) => (
        <Text strong>{getRentName(record)}</Text>
      ),
    },
    {
    ...
    },
    {
      ...
      key: "lease_end_date",
      render: (_: unknown, record: RentItem) =>
        formatDate(record.lease_end_date || record.next_due_date),
    },
    {
      title: t(
        "hr.rents.colRemainingLeaseDuration",
        "Remaining Lease Duration",
      ),
      key: "remaining_lease_duration",
      render: (_: unknown, record: RentItem) => {
        const duration =
          record.remaining_lease_duration ?? record.days_remaining;
        if (duration === null || duration === undefined) return "-";
        if (duration < 0)
          return <Badge status="error" text={formatDuration(duration, t)} />;
        if (duration <= record.reminder_days)
          return <Badge status="warning" text={formatDuration(duration, t)} />;
      ...
      render: (_: unknown, record: RentItem) => (
        <Space size="middle">
          ...
          <Tooltip title={t("common.delete", "Delete")}>
            <Popconfirm
              title={t("hr.rents.confirmDelete", "Delete this rent?")}
  ...
  return (
    <div>
        ...
        <Space wrap>
          ...
          <Select
            ...
            options={[
              ...
              {
                value: "upcoming",
                label: t("hr.rents.filter.upcoming", "Upcoming"),
              },
              {
                value: "overdue",
                label: t("hr.rents.filter.overdue", "Overdue"),
              },
            ]}
            style={{ width: 150 }}
          />
          <Select
            ...
            options={rentTypes.map((rt) => ({
              value: rt.id,
              label: language === "ar" ? rt.name_ar || rt.name_en : rt.name_en,
            }))}
            style={{ width: 200 }}
          />
          <Button
      ...
      <Card bordered={false} style={{ borderRadius: 8 }}>
        <Table
          ...
          locale={{
            emptyText: (
              <Empty
                description={t("hr.rents.empty", "No rent records found.")}
              />
            ),
          }}
          pagination={{
      ...
      <Modal
        title={
          viewing
            ? `${t("hr.rents.viewTitle", "Rent Details")}: ${getRentName(viewing)}`
            : t("hr.rents.viewTitle", "Rent Details")
        }
        open={!!viewing}
          ...
          <Space direction="vertical" size="large" style={{ width: "100%" }}>
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
              <Descriptions.Item label={t("hr.rents.colRentType", "Rent Type")}>
                {language === "ar"
                  ? viewing.rent_type?.name_ar ||
                    viewing.rent_type?.name_en ||
                    "-"
                  : viewing.rent_type?.name_en || "-"}
              </Descriptions.Item>
              <Descriptions.Item label={t("hr.rents.colName", "Name")}>
                {getRentName(viewing)}
              </Descriptions.Item>
              <Descriptions.Item label={t("hr.rents.colLocation", "Location")}>
                {viewing.property_address || "-"}
              </Descriptions.Item>
              <Descriptions.Item label={t("hr.rents.colStatus", "Status")}>
                {getStatusTag(viewing)}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.field.leaseStartDate", "Lease Start Date")}
              >
                {formatDate(viewing.lease_start_date)}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.colLeaseEndDate", "Lease End Date")}
              >
              ...
              >
                {formatDuration(
                  viewing.remaining_lease_duration ?? viewing.days_remaining,
                  t,
                )}
              </Descriptions.Item>
              <Descriptions.Item
                label={t(
                  "hr.rents.field.notificationDate",
                  "Notification Date",
                )}
              >
              ...
              <Descriptions.Item
                label={t("hr.rents.field.annualRentValue", "Annual Rent Value")}
              >
                {renderMoney(viewing.annual_rent_value)}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.field.securityDeposit", "Security Deposit")}
              >
                {renderMoney(viewing.security_deposit)}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.field.paymentSchedule", "Payment Schedule")}
              >
                {viewing.payment_schedule || "-"}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.field.autoRenewal", "Auto Renewal")}
              >
                {viewing.auto_renewal
                  ? t("common.yes", "Yes")
                  : t("common.no", "No")}
              </Descriptions.Item>
              <Descriptions.Item label={t("hr.rents.field.notice", "Notice")}>
                {viewing.notice || "-"}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.field.payments", "Payments")}
              >
                {viewing.payments || "-"}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.colRecurrence", "Recurrence")}
              >
                <Tag
                  color={viewing.recurrence === "ONE_TIME" ? "blue" : "purple"}
                >
              ...
              <Descriptions.Item
                label={t("hr.rents.colReminderDays", "Reminder (Days)")}
              >
                {t(
                  "hr.rents.reminderBefore",
                  { days: viewing.reminder_days },
                  "{days} days before",
                )}
              </Descriptions.Item>
              <Descriptions.Item
                label={t("hr.rents.colLastReminder", "Last Reminder")}
              >
                {viewing.last_reminder_sent_at
                  ? formatDate(viewing.last_reminder_sent_at)
                  : t("hr.rents.notSent", "Not Sent")}
              </Descriptions.Item>
              {isHeadOffice && (
                <Descriptions.Item
                ...
                >
                  {viewing.company_name ? (
                    <Tag color="blue">{viewing.company_name}</Tag>
                  ) : (
                    "-"
                  )}
                </Descriptions.Item>
                    ...
                    <Table
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          dataIndex: "category",
                          render: (value: RentPaymentCategory) =>
                            getPaymentCategoryLabel(value),
                        },
                        {
                          title: t("hr.rents.payments.colStatus", "Status"),
                          dataIndex: "status",
                          render: (value: RentPaymentStatus) =>
                            getPaymentStatusTag(value),
                        },
                        {
                    ...
                    >
                      <Space align="start" wrap style={{ width: "100%" }}>
                        ...
                        <Form.Item
                          name="category"
                          label={t(
                            "hr.rents.payments.fieldCategory",
                            "Category",
                          )}
                          rules={[
                            {
                      ...
                      <Space align="start" wrap style={{ width: "100%" }}>
                        <Form.Item
                          name="due_date"
                          label={t(
                            "hr.rents.payments.fieldDueDate",
                            "Due Date",
                          )}
                        >
                          ...
                          {({ getFieldValue }) =>
                            getFieldValue("status") === "paid" ? (
                              <Form.Item
                                name="paid_date"
                                label={t(
                                  "hr.rents.payments.fieldPaidDate",
                                  "Paid Date",
                                )}
                                rules={[
                                  {
                      ...
                      <Form.Item
                        name="notes"
                        label={t("hr.rents.payments.fieldNotes", "Notes")}
                      >
      ...
      <Modal
        title={t("hr.rents.createTitle", "Create Rent")}
        open={createOpen}
        onOk={handleCreate}
      ...
      >
        ...
        >
          <Form.Item
            name="rent_type_id"
            label={t("hr.rents.colRentType", "Rent Type")}
            rules={[
          ...
          >
            <Select
              options={rentTypes.map((rt) => ({
                value: rt.id,
                label:
                  language === "ar" ? rt.name_ar || rt.name_en : rt.name_en,
              }))}
            />
          </Form.Item>
          <Form.Item
            name="asset_id"
            label={t("hr.rents.field.assetOptional", "Asset (Optional)")}
          >
            <Select
              allowClear
          ...
          <Form.Item
            ...
            label={t("hr.rents.field.propertyNameEn", "Property Name (EN)")}
          >
            <Input />
          </Form.Item>
          <Form.Item
            name="property_name_ar"
            label={t("hr.rents.field.propertyNameAr", "Property Name (AR)")}
          >
            <Input />
          </Form.Item>
          <Form.Item
            name="property_address"
            label={t("hr.rents.field.propertyAddress", "Property Address")}
          >
            <Input />
          </Form.Item>
          <Form.Item
            ...
            {({ getFieldValue }) =>
              getFieldValue("recurrence") === "ONE_TIME" ? (
                <Form.Item
                  ...
                  label={t("hr.rents.colDueDate", "Due Date")}
                  rules={[
                    {
                      required: true,
                      message: t("common.required", "Required"),
                ...
                <>
                  <Form.Item
                    name="start_date"
                    label={t("hr.rents.field.startDate", "Start Date")}
                    rules={[
                      {
                        required: true,
                        message: t("common.required", "Required"),
                  ...
                  <Form.Item
                    name="due_day"
                    label={t("hr.rents.field.dueDay", "Due Day")}
                    rules={[
                      {
                        required: true,
                        message: t("common.required", "Required"),
          ...
          <Form.Item
            name="reminder_days"
            label={t("hr.rents.field.reminderDays", "Reminder Days")}
            rules={[
              { required: true, message: t("common.required", "Required") },
            ]}
          >
      ...
      <Modal
        title={t("hr.rents.editTitle", "Edit Rent")}
        open={editOpen}
        onOk={handleEdit}
      ...
      {!loading && items.length === 0 && (
        <div style={{ marginTop: 12 }}>
          <Text type="secondary">
            {t("hr.rents.empty", "No rent records found.")}
          </Text>
        </div>
      )}
    </div>
