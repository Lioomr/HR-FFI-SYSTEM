...
import { isStaleRequest, requestAgeInDays, requestAgeLabel } from "./requestAge";
...
describe("requestAgeInDays", () => {
    it("counts whole days since the request was submitted", () => {
        expect(requestAgeInDays(new Date(NOW - 3 * DAY).toISOString(), NOW)).toBe(3);
    });
    ...
    it("floors a future timestamp at zero rather than going negative", () => {
        expect(requestAgeInDays(new Date(NOW + 2 * DAY).toISOString(), NOW)).toBe(0);
    });
});

describe("requestAgeLabel", () => {
    it("uses the singular form for exactly one day", () => {
        expect(requestAgeLabel(t, new Date(NOW - DAY).toISOString(), NOW)).toBe("1 day waiting");
    });

    it("uses the plural form beyond one day", () => {
        expect(requestAgeLabel(t, new Date(NOW - 5 * DAY).toISOString(), NOW)).toBe("5 days waiting");
    });

    it("says today for a request submitted within the last day", () => {
        expect(requestAgeLabel(t, new Date(NOW - 3600_000).toISOString(), NOW)).toBe("Today");
    });
});

describe("isStaleRequest", () => {
    it("flags a request only once it passes the stale threshold", () => {
        expect(isStaleRequest(new Date(NOW - 2 * DAY).toISOString(), NOW)).toBe(false);
        expect(isStaleRequest(new Date(NOW - 3 * DAY).toISOString(), NOW)).toBe(true);
    });
