    ...
    return [
      {
        ...
        state: "completed",
        note:
          request.manual_entry_reason || t("leave.approvalMap.recordedByHr"),
      ...
      },
      {
        ...
        note:
          request.status === "rejected"
            ? decisionNote || t("leave.approvalMap.rejected")
            : t("leave.approvalMap.autoApproved"),
  ...
  const hrState: ApprovalFlowStage["state"] = finalCancelled
    ? request.decided_at || request.hr_decision_note || request.decision_reason
      ...
      : needsManager && !request.manager_decision_at
        ...
        : "upcoming"
    : finalRejected &&
        (!!request.hr_decision_note || !request.manager_decision_note)
      ? "rejected"
      : request.status === "pending_hr"
        ...
        : request.status === "pending_ceo" ||
            ...
            Boolean(request.decided_at || request.hr_decision_note)
          ? "completed"
          : (needsDelegate && !request.delegate_decision_at) ||
              (needsManager && !request.manager_decision_at)
            ? "upcoming"
            : "current";
  ...
  const stages: ApprovalFlowStage[] = [
    ...
    },
    {
      ...
      note:
        delegateState === "skipped"
          ...
          : request.delegate_decision_note ||
            t(`leave.approvalMap.${delegateState}`),
    ...
    },
    {
      ...
      detail: hrManagerName
        ? t(
            "leave.approvalMap.hrManagerName",
            { name: hrManagerName },
...
export default function LeaveApprovalMap({
  request,
  t,
}: {
  request: LeaveRequest;
