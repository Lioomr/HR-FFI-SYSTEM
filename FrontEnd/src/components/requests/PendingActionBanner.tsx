...
export default function PendingActionBanner({
  ...
  const actor =
    workflow.current_actor?.full_name || workflow.current_actor?.email;
  const stage = workflow.current_stage;

  let message: string;
  if (actor) {
    message = t(
      "pendingBanner.waitingForActor",
      { actor },
      `Waiting for ${actor}`,
    );
    ...
    const stageLabel =
      stage?.replace(/_/g, " ") || workflow.current_approver_role || "approval";
    message = t(
      "pendingBanner.waitingForStage",
      { stage: stageLabel },
      `Waiting for ${stageLabel}`,
    );
  }
