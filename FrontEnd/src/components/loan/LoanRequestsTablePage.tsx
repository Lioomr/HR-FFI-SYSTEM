...
export default function LoanRequestsTablePage({
    ...
    const load = useCallback(
        async (targetPage: number, { isRefresh = false }: { isRefresh?: boolean } = {}) => {
    ...
    const employeeName = (record: LoanRequest) =>
        record.employee?.full_name || record.employee?.email || t("manager.requests.unknown");
    ...
    const columns: ColumnsType<LoanRequest> = [
        ...
        },
        {
            ...
            width: 140,
            render: (_, record) => (
                <span className="tabular-nums" style={{ color: "#64748b", whiteSpace: "nowrap" }}>
    ...
    return (
        <ApprovalQueuePage
            ...
            emptyDescription={
                statusFilter ? t("loans.inbox.emptyFilteredDescription") : t("loans.inbox.emptyDescription")
            }
            onRetry={() => load(1)}
