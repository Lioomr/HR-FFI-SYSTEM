...
import { Alert, Button, Col, Grid, Input, Modal, Row, Space, Typography, notification } from "antd";
...
export default function LoanRequestDetailsPage({
    ...
    const employeeName = item?.employee?.full_name || item?.employee?.email || "—";
    ...
    if (error) return <ErrorState title={t("common.error")} description={error} onRetry={load} />;
    if (!item) return <ErrorState title={t("common.error")} description={t("loans.myRequests.failedLoad")} onRetry={load} />;
    ...
    return (
        <div style={{ maxWidth: 1600, margin: "0 auto", paddingBottom: 24 }}>
            ...
            <Row gutter={gutter} align="top">
                <Col xs={24} lg={14}>
                    <Space direction="vertical" size={isMobile ? 12 : 20} style={{ width: "100%" }}>
                        <DashboardPanel title={t("loans.details.employeeSection")} animDelay={0}>
                            <TeamMemberCell name={employeeName} secondary={item.employee?.email} size={44} />
                        </DashboardPanel>

                        <DashboardPanel title={t("loans.details.requestSection")} animDelay={60}>
                            <div
                                style={{
                                    display: "grid",
                            ...
                            >
                                ...
                                <Field label={t("loans.request.monthlyDeductionLabel")}>
                                    <span className="tabular-nums">{formatNumber(monthlyDeduction)}</span>
                                </Field>
                                ...
                                <Field label={t("loans.list.colCreated")}>
                                    <span className="tabular-nums">{formatDateOnly(item.created_at, "—")}</span>
                                </Field>
                            </div>
                        ...
                        <DashboardPanel title={t("loans.details.decisionSection")} animDelay={120}>
                            {canAct ? (
                                <Space size={8} wrap>
                                    <ApprovalActions
                                        size="middle"
                ...
                <Col xs={24} lg={10}>
                    ...
                    >
                        <DashboardPanel title={t("loans.details.approvalTrail")} animDelay={160}>
                            <LoanApprovalMap request={item} t={t} />
                        </DashboardPanel>

                        <DashboardPanel title={t("loans.details.historyTitle")} animDelay={200}>
            ...
            <Modal
                ...
                okButtonProps={{ loading: submitting, "aria-label": approveLabel || t("common.approve") }}
