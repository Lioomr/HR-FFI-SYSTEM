...
| Gate 4 — mobile security hardening | **In progress (2026-08-30); review sweep run and local re-authentication implemented** | Protected-binary decision stands (2026-07-30): `MobileApp` remains metadata/structured-detail only. Dependency, secret, static, transport, and deep-link review completed; all 8 high-severity advisories cleared via build-tooling overrides with the SDK 57 baseline untouched. Biometric/local re-authentication now gates cold-launch session restore and both attendance actions, and an unprotected device (no biometric, no passcode) is refused entry. **Outstanding:** certificate pinning, a device security review on real hardware, and formal sign-off. |
| Geofenced attendance (backend Phase 8 / mobile Phase 10) | **Mobile verified (2026-08-30); release validation outstanding** | Backend Phase 8 is commit `8eb0c377` (work locations, `attendance.geofence_enabled`, GPS validation on `me/check-in/` and `me/check-out/`). Mobile sends the exact three-field GPS body when a foreground fix is available and the legacy empty-body request when it is not, preserving toggle-off compatibility with exactly one POST per action. 46 suites / 399 tests, type-check, zero-warning lint, Prettier. See `plans/Geofenced Attendance Phase 8 Backend Contract.md`. Native device validation and the backend PostgreSQL geofence suite remain outstanding. |
...
| 2026-08-30 | Send GPS coordinates on every attendance action when a foreground fix exists, and fall back to the legacy empty-body request only when the device cannot produce one. | `GET /settings/` is SystemAdmin-only, so an employee client cannot read `attendance.geofence_enabled`. Choosing the request shape from the device reading keeps the backend the sole authority on geofencing while preserving the contract's empty-body compatibility when the toggle is off. The shape is chosen before the request is sent, so it is never a retry and each action makes exactly one POST. |
| 2026-08-30 | Classify the outside-work-location 403 from one verbatim sentinel and render a local translated string. | Three distinct 403s share the status on these routes, and the sanitized server-text channel is deliberately limited to 400/422. Strict equality against the single contract sentence gives the employee a dedicated, actionable state without widening that channel or retaining any server text; any reworded response degrades to the generic forbidden copy. |
| 2026-08-30 | When a device-caused legacy request is rejected with a GPS 422, report the device cause rather than the server's wording. | The employee never sent coordinates, so "invalid coordinates" is both untrue and unactionable; the permission or no-fix message tells them what to change. No backend text or location data is exposed either way. |
| 2026-08-30 | Clear the high-severity dependency advisories with targeted npm `overrides` rather than upgrading SDK packages. | Every remaining advisory is build tooling that never reaches the app bundle, and npm's proposed "fix" was a semver-major downgrade to Expo SDK 46, which `AGENTS.md` forbids. Pinning `metro`/`metro-config`/`metro-transform-worker` to the patched 0.84.5 and `brace-expansion@<=1.1.17` to 1.1.18 removed all 8 highs while leaving every direct dependency untouched, verified by a real iOS Metro export. |
| 2026-08-30 | Accept `nanoid@3.3.16` as not exploitable rather than forcing an upgrade. | It reaches runtime only through `expo-router`, which imports `nanoid/non-secure` and calls `nanoid()` with the default size. The advisory requires `customAlphabet`/`customRandom` with size `0`, which neither expo-router nor `MobileApp` uses. |
| 2026-08-30 | Require local re-authentication on cold-launch session restore and on both attendance actions. | Gate 4 requires approved biometric/local re-authentication for sensitive operations. The challenge runs inside `AuthProvider` bootstrap, so a restored session cannot reach `authenticated` without it, and inside the attendance action before any location capture or request. A fresh credential login is not re-challenged, and nothing about the challenge is persisted. |
| 2026-08-30 | Refuse to open the app on a device with no biometric and no passcode. | Superseded the initial pass-through default on the project owner's decision. An HR client holding salary and identity data must not open on an unprotected device, so `SecurityLevel.NONE` — and an unreadable enrolment state — both fail the gate. Because retrying can never succeed there, the failure carries a distinct `unavailable` reason and the lock screen tells the employee to set a device passcode rather than looping them through a futile challenge. No failure path can return a pass; a QA invariant enforces that. |
| 2026-08-30 | Render the cold-launch lock on the existing entry route rather than adding a route. | `locked` holds `initialRouteForAuthStatus` at `null`, exactly like an unreachable bootstrap, so the lock screen renders on `index` and the token-free eight-path route surface locked down at Gate 2 is preserved. |
| 2026-08-30 | Confine `expo-location` to `src/services/location/location.ts`, foreground-only, with readings held in memory for the duration of one action. | Mirrors the existing single-adapter rule for `expo-secure-store` and is enforced by a Gate 3 invariant. Background location, "always" permission, motion, geofencing tasks, and the Android foreground service are all disabled, so no location history can accumulate on the device. |
