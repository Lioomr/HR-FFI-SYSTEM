...
| Employee documents | `GET/POST /api/employees/{id}/documents/`, `PATCH/DELETE .../{document_id}/`, `POST .../{document_id}/extract/`, and `GET .../{document_id}/download/` | Current; Gate 1 verified | Owner/HR permission and active-company scope return generic 404s for IDOR/cross-company attempts. Uploads validate extension, claimed MIME, size, and PDF/JPEG/PNG signatures. Passport, Iqama/Saudi ID, and Visa OCR runs on self-hosted PaddleOCR queued to the Celery worker, with a queue claim that prevents duplicate jobs; HR/SystemAdmin can re-run extraction. Raw OCR text is stored server-side only and never serialised. `DELETE` is HRManager/SystemAdmin only, permanently removes the private file and the record together (record preserved if storage deletion fails), refuses system-generated documents, and audits `employee_document_deleted`. Downloads are audited `application/octet-stream` attachments with `nosniff` and private no-store headers. |
...
| Annual Leave settlement | `GET/POST /api/leaves/annual-leave-payments/`; `GET /api/leaves/annual-leave-payments/eligibility/`; `POST /api/leaves/annual-leave-payments/{id}/review/`; `POST .../{id}/approve/`; `POST .../{id}/reject/` | Current | Contract-date accrual is 1.75 days per completed 30-day period; employee payment requests are limited to the final 5 contract-year days, reviewed by HR, and finally decided by CEO. Eligibility is calculated server-side. HR may create a post-year-end pay/carry-forward decision for an employee when no request was submitted. All routes are authenticated and active-company scoped. |
...
carries an HR/CEO leave-approval surface (roadmap phases 1–2). Delegation and
attendance-correction approvals are now implemented in mobile roadmap phases 3
and 5; hiring-request approvals are implemented in phase 6. Loan approvals remain deferred.
...
| Delegation rules | `GET/POST /api/core/workflow/delegations/`, `GET/PATCH /{id}/` | Current; mobile Phase 3 | `DelegationRuleSerializer` fields are `from_user_id`, `to_user_id`, `start_at`, optional `end_at`, optional `reason`, and `is_active`; it has **no** `role` field. The mobile client creates rules only for the signed-in user, reads the server-scoped list, and patches only `is_active`. The backend remains authoritative for ownership and HR/SystemAdmin-wide access. |
| Delegation candidates | `GET /api/employees/delegation-candidates/` | Current; mobile Phase 3 | Authenticated, active-company-scoped candidate list; it excludes the signed-in user and returns only active login accounts for the default scope. Mobile sends no `scope=all` selector and uses this list only to choose `to_user_id`. |
...
- Loans, assets, agent memory, and the HR/admin/approval families are outside the first employee mobile release unless the master plan is changed explicitly. **Amended 2026-08-28:** it now includes Leave, attendance-correction, hiring-request, and loan approval routes (phases 2, 5, 6, and 7). Assets and agent memory remain out of scope entirely.
